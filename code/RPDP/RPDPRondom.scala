...
object RPDPRondom {
  def main(args: Array[String]): Unit = {
    ...
    def sendMsg(ec:EdgeContext[Array[ArrayBuffer[String]],Int,Array[ArrayBuffer[String]]]):Unit={
      ...
      if(ec.srcAttr!=null){
        for(i<-ec.srcAttr){
          ...
          {
            arrayBuffer += ArrayBuffer(ec.dstId.toString ,i(1).toString ,(i(2).toInt+1).toString ,0.toString)
          }
        }
      }
    ...
    val initialGraph=graphReverse.mapVertices((id,attr)=>(Array(ArrayBuffer(id.toString,id.toString,0.toString,1.toString))))
    //验证map中的元素
    val a=propageteEdge(initialGraph).cache()
    ...
    val newGraph=a1.mapVertices({case(id,(attr))=>
      ...
      attr.foreach(f => {
        ...
        if ((f(2).toInt)==1) {
          println("f是多少"+f)
          //random产生的随机数0-9
          if (random01.nextInt(9) > 7) {
            //寻找匹配的边
            attr.foreach(f1 =>
              if (((f1(2).toInt))!= 0 && ((f1(2).toInt)) != 1) {
                arrayBufferEdge += (f1(0).toString + f1(1).toString)
